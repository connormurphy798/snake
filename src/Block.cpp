...
bool Block::incrementSPos(Vector2 p_d, const int* p_limits) {
    Vector2 spos_buffer = f_spos + p_d;
    int x = Utils::mid(p_limits[2], spos_buffer.f_x, p_limits[3]);
    int y = Utils::mid(p_limits[0], spos_buffer.f_y, p_limits[1]);
    setSPos(Vector2(x,y));
    updatePos();
